...
const GAMES = [
  ...
  { n:'시크릿 히틀러', img:'img/t/secret-hitler.jpg', min:5, max:10, t:45, cat:'mafia', lv:'중급', out:false, host:false, own:'임햇살', have:true,
  ...
  { n:'아발론', img:'img/t/avalon.jpg', min:5, max:10, t:30, cat:'mafia', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'원탁의 기사 안에 숨은 모드레드의 하수인을 찾아라.',
    r:'원정대를 꾸려 임무를 보낸다. 선이 3번 성공하면 승리, 악이 3번 방해하면 패배. 멀린은 악을 알지만 들키면 안 된다.' },
  { n:'스파이폴2', img:'img/t/spyfall2.jpg', min:3, max:12, t:20, cat:'mafia', lv:'초보', out:false, host:false, own:'채연주', have:true,
  ...
  { n:'사보타지', img:'img/t/saboteur.jpg', min:3, max:10, t:30, cat:'mafia', lv:'초보', out:false, host:false, own:'', have:true,
    d:'금을 캐는 광부 사이에 길을 막는 방해꾼이 섞여 있다.',
    r:'터널 카드를 이어 금까지 길을 낸다. 광부는 도착하면 승리, 방해꾼은 막으면 승리. 서로 정체를 모른 채 진행.' },

  // ── 파티 ──────────────────────────────────
  { n:'힛스터', img:'img/t/hitster.jpg', min:2, max:null, t:30, cat:'party', lv:'초보', out:false, host:false, own:'임햇살', have:true,
    d:'노래 듣고 연도 맞히기. 팀으로 나누면 인원 제한이 없다.',
    r:'QR을 찍어 노래를 틀고 발매 연도를 추측해 내 연표 사이에 끼워넣는다. 맞으면 카드 획득, 10장 모으면 승리.' },
  { n:'달무티', img:'img/t/dalmuti.jpg', min:4, max:8, t:15, cat:'party', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'신분 계급이 매 판 뒤집힌다. 왕이 거지가 되는 맛.',
    r:'낮은 숫자가 강하다. 카드를 먼저 다 버린 순서대로 다음 판의 계급이 정해지고, 계급에 따라 카드를 상납한다.' },
  { n:'플립7', img:'img/t/flip7.jpg', min:3, max:18, t:20, cat:'party', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'한 장 더? 여기서 멈춤? 18명까지 되는 심플 도박.',
    r:'카드를 계속 뽑아 점수를 쌓되 같은 숫자가 겹치면 그 판 점수는 0. 서로 다른 숫자 7장을 모으면 보너스.' },
  { n:'애니멀렐름', img:'img/t/animal-realms.jpg', min:3, max:7, t:15, cat:'party', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'동물 카드로 벌이는 짧은 세력 다툼.',
    r:'같은 동물을 모으거나 상대를 밀어내며 영역을 차지한다. 한 판이 짧아 여러 번 돌린다.' },
  { n:'바퀴벌레포커', img:'img/t/cockroach-poker.jpg', min:3, max:5, t:20, cat:'party', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'거짓말 하나로 굴러가는 게임. 마피아 몸풀기에 좋다.',
    r:'카드를 뒤집어 내밀며 "이건 바퀴벌레"라고 말한다. 받는 사람은 진실인지 거짓인지 판정하고, 틀린 쪽이 카드를 가져간다. 같은 벌레 4장을 모으면 패배.' },
  { n:'조겐 (Zogen)', img:'img/t/zogen.jpg', min:2, max:6, t:20, cat:'party', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'가볍게 굴러가는 카드 게임.',
    r:'카드를 내며 조건을 맞춘다. 룰이 짧아 설명 시간이 거의 안 든다.' },
  { n:'케자오 (Kezao)', img:'img/t/kezao.jpg', min:2, max:5, t:20, cat:'party', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'짧고 가벼운 카드 게임.',
    r:'조건에 맞는 카드를 빠르게 내며 손패를 턴다.' },
  { n:'파티사우루스20s', img:'img/t/partysaurus.jpg', min:3, max:5, t:20, cat:'party', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'공룡 테마 파티게임.',
  ...
  { n:'워드캡처', img:'img/t/word-capture.jpg', min:2, max:6, t:10, cat:'party', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'10분짜리 단어 게임. 대기 시간 메우기에 최적.',
    r:'제시된 조건에 맞는 단어를 먼저 잡는다. 순발력과 어휘력 싸움.' },

  // ── 순발력 ────────────────────────────────
  { n:'타코캣고트치즈피자', img:'img/t/tacocat.jpg', min:3, max:8, t:10, cat:'quick', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'10분. 시끄럽고, 아무도 지루할 틈이 없다.',
    r:'"타코-캣-고트-치즈-피자"를 순서대로 외치며 카드를 낸다. 말과 카드가 일치하면 손을 올린다. 가장 늦은 사람이 카드 더미를 가져간다.' },
  { n:'할리갈리', img:'img/t/halli-galli.jpg', min:2, max:6, t:15, cat:'quick', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'설명이 필요 없는 국민 게임.',
  ...
  { n:'서펜티나', img:'img/t/serpentina.jpg', min:2, max:5, t:15, cat:'quick', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'뱀을 이어붙이는 짧은 게임. 아이도 어른도 된다.',
    r:'카드를 뒤집어 색이 맞는 뱀을 이어붙인다. 머리와 꼬리를 완성하면 그 뱀을 가져간다.' },
  { n:'아브라카왓', img:'img/t/abracawhat.jpg', min:2, max:5, t:30, cat:'quick', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'주문을 외우며 물건을 집는 마법 테마 순발력 게임.',
    r:'조건에 맞는 물건을 먼저 집는다. 라운드마다 조건이 바뀌어 헷갈린다.' },

  // ── 전략 ──────────────────────────────────
  { n:'클루', img:'img/t/clue.jpg', min:2, max:6, t:20, cat:'strategy', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'누가, 어디서, 무엇으로. 추리 게임의 고전.',
    r:'질문과 소거로 범인·장소·흉기를 좁혀간다. 먼저 정확히 맞히면 승리.' },
  { n:'코드777', img:'img/t/code777.jpg', min:2, max:5, t:40, cat:'strategy', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'남의 숫자는 보이는데 내 숫자만 안 보인다.',
    r:'이마에 붙은 내 숫자를 남들의 반응과 질문 카드로 추론한다. 먼저 맞히면 승리.' },
  { n:'카르카손', img:'img/t/carcassonne.jpg', min:2, max:5, t:35, cat:'strategy', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'타일을 놓아 도시와 길을 만드는 영역 싸움.',
    r:'타일을 이어 붙이고 부하를 올려 도시·길·수도원을 완성한다. 완성된 지형의 크기만큼 점수.' },
  { n:'스플렌더', img:'img/t/splendor.jpg', min:2, max:4, t:30, cat:'strategy', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'보석을 모아 카드를 사고, 그 카드로 또 산다.',
    r:'토큰을 모아 개발 카드를 구입한다. 카드는 영구 보너스가 되어 다음 구입이 쉬워진다. 15점 먼저 도달하면 승리.' },
  { n:'카탄', img:'img/t/catan.jpg', min:3, max:4, t:60, cat:'strategy', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'자원 교환과 협상. 유로게임의 교과서.',
    r:'주사위로 자원을 얻고 길·마을·도시를 건설한다. 부족한 자원은 다른 사람과 협상해 교환. 10점 먼저 도달하면 승리.' },
  { n:'카탄 (카드버전)', img:'img/t/catan-card.jpg', min:2, max:4, t:30, cat:'strategy', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'카탄을 카드로 압축한 버전. 시간이 절반.',
    r:'카드로 자원을 관리하며 자기 영지를 키운다. 보드판 없이 진행.' },
  { n:'루미큐브', img:'img/t/rummikub.jpg', min:2, max:4, t:20, cat:'strategy', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'숫자 타일로 하는 조합 게임. 룰이 직관적이다.',
    r:'같은 색 연속 또는 같은 숫자 세트를 만들어 내려놓는다. 판 위의 조합을 재배열해도 된다. 먼저 다 내려놓으면 승리.' },
  { n:'하모니즈', img:'img/t/harmonies.jpg', min:1, max:4, t:30, cat:'strategy', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'조각을 쌓아 자연 지형을 만드는 퍼즐형 게임.',
    r:'색 토큰을 규칙에 맞게 쌓아 서식지를 만들고, 동물 카드 조건을 충족해 점수를 얻는다.' },
  { n:'봄버스터즈', img:'img/t/bomb-busters.jpg', min:2, max:5, t:30, cat:'strategy', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'폭탄을 피하며 점수를 쌓는 카드 게임.',
    r:'카드를 뽑아 점수를 모으되 폭탄이 터지면 잃는다. 언제 멈출지 판단하는 게 핵심.' },
  { n:'상자아이', img:'img/t/box-child.jpg', min:3, max:5, t:40, cat:'strategy', lv:'중급', out:false, host:false, own:'채연주', have:true,
    d:'상자 속 정보를 다루는 중량감 있는 게임.',
    r:'제한된 정보로 상대의 수를 읽으며 점수를 쌓는다.' },
  { n:'쿼리도', img:'img/t/quoridor.jpg', min:2, max:4, t:15, cat:'strategy', lv:'초보', out:false, host:false, own:'채연주', have:true,
  ...
  { n:'더마인드', img:'img/t/the-mind.jpg', min:2, max:4, t:20, cat:'coop', lv:'초보', out:false, host:false, own:'채연주', have:true,
    d:'말하지 않고 숫자 순서를 맞춘다. 이상하게 통한다.',
    r:'각자 든 숫자 카드를 대화 없이 작은 순서대로 낸다. 순서가 틀리면 목숨이 깎인다.' },
  { n:'사건의재구성', img:'img/t/crime-scene.jpg', min:1, max:4, t:75, cat:'coop', lv:'고급', out:false, host:false, own:'채연주', have:true,
